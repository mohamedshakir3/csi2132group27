...
export default function SearchBar() {
  ...
  const redirect = () => {
    if (from === "" || to === "") return alert("Please specify the dates.")
    Router.push({
    ...
    <>
      <div className={Searchbar.searchBar}>
        ...
        <div className={Searchbar.selector}>
          ...
          <input
            className={`${Searchbar.custom_date} ${Neumorphic.selector_box_shadow}`}
            type="date"
            id="from"
            name="from"
            onChange={() => setFrom(event.target.value)}
          />
        </div>
        <div className={Searchbar.selector}>
      ...
      <button className={Searchbar.search_button} onClick={redirect}>
        Search
      </button>
    </>
  )
