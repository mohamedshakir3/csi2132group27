...
export async function getServerSideProps() {
  const customerRes = await fetch(`${process.env.API_URL}/api/getCustomers`)
  const customerData = await customerRes.json()
  const customers = customerData.results

  const bookingRes = await fetch(`${process.env.API_URL}/api/getBookings`)
  const bookingData = await bookingRes.json()
  const bookings = bookingData.results

  const rentingRes = await fetch(`${process.env.API_URL}/api/getRentings`)
  const rentingData = await rentingRes.json()
  const rentings = rentingData.results

  const roomRes = await fetch(`${process.env.API_URL}/api/getRooms`)
